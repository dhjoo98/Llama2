...
#next time monkey patching, copy the source from the package version you are using. 
from accelerate.state import AcceleratorState
from accelerate.utils.constants import SAFE_WEIGHTS_NAME, WEIGHTS_NAME
from accelerate.utils.dataclasses import AutocastKwargs, CustomDtype, DistributedType
#from accelerate.utils.imports import is_mps_available, is_npu_available, is_peft_available, is_torch_xla_available, is_xpu_available
from accelerate.utils.imports import is_mps_available, is_npu_available, is_peft_available, is_xpu_available
from accelerate.utils.offload import load_offloaded_weight, offload_weight, save_offload_index
#note that no annotation needs to be done on offload_weight() so we just use accelerate's fn.
...
def load_offloaded_weights(model, index, offload_folder):
    ...
    """
    nvtx.range_push('load_offloaded_weights')
...
def load_state_dict(checkpoint_file, device_map=None):
    ...
    if checkpoint_file.endswith(".safetensors"):
        ...
        if device_map is None:
            ...
            devices = list(set(device_map.values()) - {"disk"}) #why remove disk? 
            ...
            for module_name, device in device_map.items():
                if device in devices:
                    #.extend add elements(can be multiple!) of an iterable to the end of another list
        ...
        # a looong fall back for all CPU. 
...
def load_checkpoint_in_model(
    ...
    #donghyeon: these are all invalid input checking
    ...
    if offload_state_dict:
        state_dict_folder = tempfile.mkdtemp() #create a temporary dir(returns path) > must delete later
        state_dict_index = {}

    buffer_names = [name for name, _ in model.named_buffers()]
    nvtx.range_push('load_state_dict - set tensor to device loop') #(per ckpt file)
    for checkpoint_file in checkpoint_files:
        nvtx.range_push('load_state_dict')
        checkpoint = load_state_dict(checkpoint_file, device_map=device_map)
        nvtx.range_pop() 
        #donghyeon: so load here, sharding checkpoint may reduce memory usage of an instant.
        #load_state_dict() is also overloaded here. [L.1376]
        if device_map is None:
            model.load_state_dict(checkpoint, strict=False)
        #1)we have a device map, 2)load_state_dict returned tensor dict of weights per device, 
        else: #but this, now iterates over each item.
            for param_name, param in checkpoint.items():
                #nvtx.range_push('checking if this iterates over each layer'): check
                ...
                if param_device == "disk":
                    if offload_buffers or param_name not in buffer_names:
                        ...
                        nvtx.range_push('offloading to disk')
                    ...
                    if offload_8bit_bnb:
                        ...
                        set_module_tensor_to_device(model, param_name, "meta", dtype=new_dtype)
                        nvtx.range_push('offloading to cpu: check if this is disk offloaded')
    ...
    if offload_state_dict: #yes, this branch is also taken.
        nvtx.range_push("load_offloaded_weights is taken")
