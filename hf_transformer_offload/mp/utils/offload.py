...
def offload_weight(weight, weight_name, offload_folder, index=None):
    ...
    file_array = np.memmap(tensor_file, dtype=array.dtype, mode="w+", shape=array.shape) #initialize memory region
    file_array[:] = array[:] #populate, 
    file_array.flush() #write back the dirty (written) part back to disk
    return index
